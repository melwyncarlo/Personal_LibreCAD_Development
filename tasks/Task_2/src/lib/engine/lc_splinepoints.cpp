...
namespace {
/* This is the quadratic Bezier curve function, B(t). */
RS_Vector GetQuadPoint(const RS_Vector& P0, const RS_Vector& P1, const RS_Vector& P2, double t)
{
   /*
   ...
   */
	...
	return P1 + ((P0 - P1) * (1.0 - t) * (1.0 - t)) + ((P2 - P1) * t * t);
}
	...
	double dStartT, int *piSeg, double *pdt) const
{
	...
	i++;
std::cout<<"==="<<std::endl;
std::cout<<"dQuadDist = "<<dQuadDist<<", and dDist = "<<dDist<<std::endl;
std::cout<<vStart.x<<", "<<vStart.y<<std::endl;
std::cout<<vControl.x<<", "<<vControl.y<<std::endl;
std::cout<<vEnd.x<<", "<<vEnd.y<<std::endl;
std::cout<<" |"<<std::endl;
...
std::cout<<"dt = "<<dt<<", and i = "<<i-1<<std::endl;
std::cout<<" |"<<std::endl;
...
//std::cout << vRes.x << ", " << vRes.y << ", " << vRes.z << std::endl;
    i = 2;
	while(vRes.valid && i <= middlePoints)
	{//std::cout << iNext << " & " << dt << std::endl;
		vNext = GetSplinePointAtDist(dDist, iNext, dt, &iNext, &dt);
		dCurDist = (vNext - coord).magnitude();
//std::cout << vNext.x << ", " << vNext.y << ", " << vNext.z << std::endl;
		if(vNext.valid && dCurDist < dMinDist)
		{
			dMinDist = dCurDist;
			vRes = vNext;
		}
//if (!vNext.valid) {std::cout << "1";}
//if (dCurDist >= dMinDist) {std::cout << " & 2" << std::endl;}
		i++;
	}
/*RS_Vector k1, k2, k3;
std::cout << "n = " << data.controlPoints.size() << std::endl;
for (int z = 0; z < data.controlPoints.size(); z++)
{
std::cout << data.controlPoints.at(z) << ", " << std::endl;
}
std::cout << std::endl;
for (int z = 1; z <= (data.controlPoints.size()-1); z++)
{
int k = GetQuadPoints(z, &k1, &k2, &k3);
std::cout << "=> " << z <<": " << std::endl;
std::cout << "k = " << k << std::endl;
std::cout << k1.x << ", " << k1.y << ", " << k1.z << std::endl;
std::cout << k2.x << ", " << k2.y << ", " << k2.z << std::endl;
std::cout << k3.x << ", " << k3.y << ", " << k3.z << std::endl;
}*/
//std::cout << getLength() << " vs. my " << Bezier_Curve_Length(0.0, 1.0) << std::endl;
std::cout << std::endl;
	if(dist) *dist = dMinDist;

   /*int controlPointsFirstSet = 0;
   int controlPointsLastSet  = data.controlPoints.size() - 3;

   std::cout << getLength() << std::endl;
   for (int i = controlPointsFirstSet; i <= controlPointsLastSet; i++)
   {
      vStart   = data.controlPoints.at(i);
      vControl = data.controlPoints.at(i + 1);
      vEnd     = data.controlPoints.at(i + 2);

      int t = 1.0;

      std::cout << (vStart * (t*t*t)) + (vEnd * (t*t*t)) + (t*t*t) - (vStart*2*t*t) + (vStart*t) - (t*t*t*t) << std::endl;
   }
   std::cout << std::endl;*/
