...
void LC_ActionFactory::fillActionContainer(QMap<QString, QAction*>& a_map, LC_ActionGroupManager* agm)
{
    ...
    /* Snap Biliear Intersection */
    action = new QAction(tr("Snap Bilinear Intersection"), agm->line);
    connect(action, &QAction::triggered, action_handler, &QG_ActionHandler::slotSnapBilinearIntersection);
    action->setObjectName("SnapBilinearIntersection");
    a_map["SnapBilinearIntersection"] = action;
